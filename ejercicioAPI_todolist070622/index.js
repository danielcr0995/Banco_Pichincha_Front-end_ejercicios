...
const setTodoList = async () => {
    ...
    var cointainerList = document.querySelector('.container-list');
    cointainerList.innerHTML='';
    console.log(json)
    
    for(let i = 0; i < json.data.length; i++){
        const element = json.data[i];
        cointainerList.innerHTML += `
            <div class="item" data-id="${element.id}" id="item-element_${element.id}">
            <div class="item-content">
                <h2>${element.description}</h2>
            </div>
            <div class="item-actions">
                <!-- SVG Edit  -->
                <button class="action-button" id="item-edit${element.id}">
